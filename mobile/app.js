...
function envoyerCommandeTexte() {
  const text = (textCommandInput.value || "").trim();
  if (!text) return;
  afficherTexteUtilisateur(text);
  textCommandInput.value = "";
  applyState("thinking");
  const envoye = sendCommand(text);
  if (!envoye) {
    applyState("idle");
    userTextEl.textContent = "⚠ Non connecte a JARVIS";
  }
}
